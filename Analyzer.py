...
def testCase(strin, trials):
	...
	result += "="*20 + "\n"
	result += "TOTAL TIME: " + str(nResults["TotalTime"]) + "="*3 + ">" + str(oResults["TotalTime"]) + "="*3 + ">" + "% Improvement: " + str(round((nResults["TotalTime"]-oResults["TotalTime"])/nResults["TotalTime"] if nResults["TotalTime"] != 0 else 1.000, 3)) + "\n"
	result += "AVERAGE TIME: " + str(nResults["AverageTime"]) + "="*3 + ">" + str(oResults["AverageTime"]) + "="*3 + ">" + "% Improvement: " + str(round((nResults["AverageTime"]-oResults["AverageTime"])/nResults["AverageTime"] if nResults["AverageTime"] != 0 else 1.000, 3)) + "\n"
